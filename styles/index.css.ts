import { boxStyles, stackStyles, rowStyles, clampStyles, columnsStyles, gridStyles, splitStyles, toSelectorString } from "@christiankaindl/lyts"
...
const boxClassName = toSelectorString(boxStyles.box)
...
globalStyle(`${debug} ${toSelectorString(boxStyles.box)}`, {
...
globalStyle(`${debug} ${toSelectorString(boxStyles.box)}:not(${toSelectorString(stackStyles.stack)}):not(${toSelectorString(rowStyles.row)}):not(${toSelectorString(clampStyles.clamp)}):not(${toSelectorString(columnsStyles.columns)}):not(${toSelectorString(gridStyles.grid)})`, {
  boxShadow: 'rgb(0 0 0 / 0.15) 0 3px 15px -5px, rgb(0 0 0 / 0.04) 0 0 0 1px',
})
globalStyle(`${debug} ${toSelectorString(boxStyles.box)} > ${toSelectorString(boxStyles.box)}`, {
  borderRadius: 12
})

globalStyle(`${debug} ${toSelectorString(stackStyles.stack)}`, {
  borderColor: redA.redA4,
  backgroundImage: `linear-gradient(160deg, ${tomatoA.tomatoA2}, ${crimsonA.crimsonA2})`
})

globalStyle(`${debug} ${toSelectorString(rowStyles.row)}`, {
  borderColor: pinkA.pinkA4,
  backgroundImage: `linear-gradient(160deg, ${crimsonA.crimsonA2}, ${plumA.plumA2})`
})

globalStyle(`${debug} ${toSelectorString(clampStyles.clamp)}`, {
  borderColor: purpleA.purpleA4,
  backgroundImage: `linear-gradient(160deg, ${plumA.plumA2}, ${violetA.violetA2})`
})

globalStyle(`${debug} ${toSelectorString(columnsStyles.columns)}`, {
  borderColor: indigoA.indigoA4,
  backgroundImage: `linear-gradient(160deg, ${violetA.violetA2}, ${blueA.blueA2})`
})

globalStyle(`${debug} ${toSelectorString(gridStyles.grid)}`, {
  borderColor: cyanA.cyanA4,
  backgroundImage: `linear-gradient(160deg, ${blueA.blueA2}, ${tealA.tealA2})`
})
globalStyle(`${debug} ${toSelectorString(splitStyles.split)}`, {
  border: `1px solid ${mauve.mauve8}`,
  padding: 0,
  height: 0,
  position: 'relative'
})
globalStyle(`${debug} ${toSelectorString(splitStyles.split)}::before, ${debug} ${toSelectorString(splitStyles.split)}::after`, {
  content: '',
  display: 'block',
  height: 12,
  width: 1,
...
globalStyle(`${debug} ${toSelectorString(splitStyles.split)}::after`, {
  left: 'initial',
  right: -1
})
